...
"""
sm4_app_updated.py
Enhanced CustomTkinter UI with detailed tooltips, descriptions, and better UX.
...
from customtkinter import CTkLabel, CTkButton, CTkEntry, CTkTextbox, CTkFrame, CTkSegmentedButton
...
class SM4App(ctk.CTk):
    def __init__(self) -> None:
        super().__init__()
        self.title("🔐 SM4 Encryption — Сучасна утиліта для шифрування")
    ...
    def _build_ui(self) -> None:
        ...
        title = CTkLabel(title_frame, text="🔐 SM4 Encryption Утиліта", 
                        font=("Segoe UI", 24, "bold"), text_color=self.text_color)
        title.pack(anchor="w")
        
        subtitle = CTkLabel(title_frame, 
                           text="Безпечне шифрування текстів та файлів за китайським стандартом SM4 (128-біт)", 
        ...
        # Content area
        self.content = CTkFrame(main, fg_color=self.bg_color)
    ...
    def _build_text_tab(self):
        ...
        # Info box
        info_box = CTkFrame(f, fg_color=self.info_color, corner_radius=8)
        info_box.pack(fill="x", pady=(0, 12))
        
        info_title = CTkLabel(info_box, text="ℹ️ Як користуватися режимом Текст", 
                             font=("Segoe UI", 11, "bold"), text_color=self.text_color)
        info_title.pack(anchor="w", padx=12, pady=(10, 4))
        
        info_text = CTkLabel(
            info_box, 
            text="1️⃣  Введіть або вставте текст у поле нижче\n"
                 "2️⃣  Введіть/сгенеруйте ключ (32 HEX символи = 16 байтів)\n"
                 "3️⃣  Натисніть 'Зашифрувати' для шифрування\n"
                 "4️⃣  Результат буде у форматі HEX",
            font=("Segoe UI", 10),
        ...
        # Input section
        in_sec = CTkFrame(f, fg_color="white", border_width=1, border_color="#D0D0D0", corner_radius=8)
        in_sec.pack(fill="x", pady=(0, 10))
        
        in_header = CTkFrame(in_sec, fg_color="white")
        in_header.pack(fill="x", padx=12, pady=(10, 0))
        
        in_lbl = CTkLabel(in_header, text="📝 Вхідний текст", font=("Segoe UI", 12, "bold"))
        in_lbl.pack(side="left")
        
        q_mark = CTkLabel(in_header, text="❓", font=("Segoe UI", 13))
        q_mark.pack(side="left", padx=(6, 0))
        create_tooltip(q_mark, "Вводьте будь-який текст\nМаксимальна довжина не обмежена")
        
        self.text_input = CTkTextbox(in_sec, height=120, font=("Segoe UI", 11))
        self.text_input.pack(fill="both", padx=12, pady=(6, 12))
        
        # Key section
        key_sec = CTkFrame(f, fg_color="white", border_width=1, border_color="#D0D0D0", corner_radius=8)
        key_sec.pack(fill="x", pady=(0, 10))
        
        key_header = CTkFrame(key_sec, fg_color="white")
        key_header.pack(fill="x", padx=12, pady=(10, 0))
        
        key_lbl = CTkLabel(key_header, text="🔑 Ключ шифрування", font=("Segoe UI", 12, "bold"))
        key_lbl.pack(side="left")
        
        key_q = CTkLabel(key_header, text="❓", font=("Segoe UI", 13))
        ...
        # Buttons
        btn_frame = CTkFrame(f, fg_color=self.bg_color)
        ...
        # Output section
        out_sec = CTkFrame(f, fg_color="white", border_width=1, border_color="#D0D0D0", corner_radius=8)
        out_sec.pack(fill="both", expand=True)
        
        out_header = CTkFrame(out_sec, fg_color="white")
        out_header.pack(fill="x", padx=12, pady=(10, 0))
        
        out_lbl = CTkLabel(out_header, text="📤 Результат (HEX формат)", font=("Segoe UI", 12, "bold"))
        out_lbl.pack(side="left")
        
        out_q = CTkLabel(out_header, text="❓", font=("Segoe UI", 13))
        ...
        copy_info = CTkLabel(out_header, text="(натисніть Ctrl+A для копіювання)", 
                            font=("Segoe UI", 9), text_color="#999999")
        copy_info.pack(side="right")
        
        self.text_output = CTkTextbox(out_sec, height=150, font=("Courier", 10))
    ...
    def _build_file_tab(self):
        ...
        info_title = CTkLabel(info_box, text="ℹ️ Як користуватися режимом Файли", 
                             font=("Segoe UI", 11, "bold"), text_color=self.text_color)
        info_title.pack(anchor="w", padx=12, pady=(10, 4))
        
        info_text = CTkLabel(
            info_box, 
            text="1️⃣  Виберіть файл для шифрування\n"
                 "2️⃣  Згенеруйте новий ключ або завантажте існуючий\n"
                 "3️⃣  Натисніть 'Зашифрувати' — файл буде збережено з розширенням .enc\n"
                 "4️⃣  Для розшифрування виберіть файл .enc і вкажіть ключ\n"
        ...
        # File selection
        file_frame = CTkFrame(f, fg_color="white", border_width=1, border_color="#D0D0D0", corner_radius=8)
        file_frame.pack(fill="x", pady=(0, 10))
        
        file_header = CTkFrame(file_frame, fg_color="white")
        file_header.pack(fill="x", padx=12, pady=(10, 0))
        
        file_lbl = CTkLabel(file_header, text="📁 Вибір файлу", font=("Segoe UI", 12, "bold"))
        file_lbl.pack(side="left")
        
        file_q = CTkLabel(file_header, text="❓", font=("Segoe UI", 13))
        ...
        browse_btn = CTkButton(file_frame, text='📂 Обрати файл', command=self._browse_file, 
                              fg_color=self.accent_color, hover_color="#005A9E", 
                              font=("Segoe UI", 10), height=36)
        browse_btn.pack(side='right', padx=12, pady=10)
        create_tooltip(browse_btn, "Відкрити діалог для вибору файлу")
        
        # Key management
        key_frame = CTkFrame(f, fg_color="white", border_width=1, border_color="#D0D0D0", corner_radius=8)
        key_frame.pack(fill="x", pady=(0, 10))
        
        key_header = CTkFrame(key_frame, fg_color="white")
        key_header.pack(fill="x", padx=12, pady=(10, 0))
        
        key_lbl = CTkLabel(key_header, text="🔑 Управління ключем", font=("Segoe UI", 12, "bold"))
        key_lbl.pack(side="left")
        ...
        # Action buttons
        action_frame = CTkFrame(f, fg_color=self.bg_color)
        ...
        dec_btn = CTkButton(action_frame, text='🔓 Розшифрувати файл', command=self._decrypt_file, 
                           ...
                           font=("Segoe UI", 11), height=44)
        dec_btn.pack(fill='x')
        create_tooltip(dec_btn, "Розшифрувати файл з розширенням .enc")
    ...
    def _gen_key(self):
        try:
            k = generate_key()
            self.enc_key = k
            short = k.hex()[:16] + "..." if len(k.hex()) > 16 else k.hex()
            self.key_label.configure(text=f"🔑 {short}")
    ...
    def _encrypt_file(self):
        ...
        try:
            ...
            out = self.enc_file.with_suffix(self.enc_file.suffix + '.enc')
            out.write_bytes(ct)
    ...
    def _decrypt_file(self):
        p = filedialog.askopenfilename(title='Виберіть зашифрований файл (.enc)', 
